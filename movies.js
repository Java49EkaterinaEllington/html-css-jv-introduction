...
///надо получить массив всех секций.
const sectionElements = document.querySelectorAll("section"); /// получаем все секции.
...
function show(index)
{
    sectionElements.forEach(section => section.hidden = true); /// закрываем
    sectionElements[index].index = false; /// открываем
}
/// forEach потому что мы меняем все секции. 
